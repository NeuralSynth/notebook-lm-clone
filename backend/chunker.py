...
- Sentence boundary snapping: after computing the raw split point, we walk
  forward/backward to the nearest sentence end ('. ', '! ', '? ') so chunks
...
def _snap_to_sentence_end(text: str, pos: int, window: int = 100) -> int:
    """
...
def chunk_text(text: str, doc_id: str, filename: str, page: int | None = None) -> List[Chunk]:
    """
    ...
    while start < len(text):
        ...
        if end < len(text):
            ...
            end = _snap_to_sentence_end(text, end)
...
def chunk_pages(pages: List[dict], doc_id: str, filename: str) -> List[Chunk]:
    """
